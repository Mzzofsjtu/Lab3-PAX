...
class MiniObServer:
  ...
  '''
    ...
    def __init__(self, base_dir: str, data_dir: str, config_file: str,
                 server_port: int, server_socket: str, clean_data_dir: bool):
    ...
    def __check_base_dir(self, base_dir: str):
        if not (os.path.isdir(base_dir)):
            raise (Exception("failed to check base directory. " + base_dir +
                             " is not a directory"))
    ...
    '''
        ...
        observer_command = [
            self.__observer_path(self.__base_dir), '-f', self.__config
        ]
        ...
        process = subprocess.Popen(observer_command,
                                   stdout=subprocess.DEVNULL,
                                   stderr=subprocess.DEVNULL,
                                   cwd=self.__data_dir)
        return_code = process.poll()
        if return_code != None:
            _logger.error("Failed to start observer, exit with code %d",
                          return_code)
            return False
    ...
    def stop_server(self):
        ...
        _logger.info("miniob-server exit with code %d. pid=%s", return_code,
    ...
    def __check_unix_socket_server(self):
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as s:
            ...
            if errno == 0:
                ...
                _logger.debug("Failed to connect to server. err=%d:%s", errno,
                              os.strerror(errno))
                return False
  ...
  '''
    ...
    def __init__(self,
        ...
        if sock != None:
            ...
            self.__poller.register(
                self.__socket, select.POLLIN | select.POLLPRI | select.POLLHUP
                | select.POLLERR)
    ...
    def __recv_response(self):
        ...
        while True:
            ...
            if len(events) == 0:
                raise Exception('Poll timeout after %d second(s)' %
                                self.__time_limit)
            ...
            if len(data) > 0:
                result_tmp = data.decode(
                    encoding=GlobalConfig.default_encoding)
                _logger.debug("receive from server[size=%d]: '%s'", len(data),
                              result_tmp)
                ...
                _logger.info("receive from server error. result len=%d",
                             len(data))
                raise Exception(
                    "receive return error. the connection may be closed")
    ...
    def run_sql(self, sql: str) -> Tuple[bool, str]:
        try:
            ...
            _logger.debug("send command to server(size=%d) '%s'",
                          len(data) + 1, sql)
...
class CommandRunner:
    ...
    def __init__(self, result_writer: ResultWriter, server_port: int,
                 unix_socket: str):
...
class TestCaseLister:
  ...
  '''
    ...
    def list_all(self, base_dir, test_names) -> List[TestCase]:
        is_dir = os.path.isdir(base_dir)
        if False == is_dir:
            raise ("Failed to list all test cases. " + base_dir +
                   " is not a directory")
...
class TestSuite:
    ...
    def set_test_result_tmp_dir(self, test_result_tmp_dir: str):
        ...
        if not (os.path.isdir(test_result_tmp_dir)):
            raise (Exception("Failed to set test result temp directory. " +
                             test_result_tmp_dir +
                             " is not a directory or failed to create"))
    ...
    def __run_case(self, test_case: TestCase) -> int:
        result_tmp_file_name = test_case.tmp_result_file(
            self.__test_result_tmp_dir)
        ...
        with open(result_tmp_file_name, mode='wb') as result_file:
            ...
            with CommandRunner(result_writer, self.__server_port,
                               unix_socket) as command_runner:
                ...
                for command_line in test_case.command_lines():
                    ...
                    if result is False:
                        _logger.error("Failed to run command %s in case %s",
                                      command_line, test_case.get_name())
                        return result
        ...
        if self.__report_only:
            ...
            result = self.__compare_files(result_tmp_file_name,
                                          result_file_name)
    ...
    def run(self, eval_result: EvalResult):
        ...
        for test_case in test_cases:
            try:
                ...
                if result is Result.true:
                    ...
                    success_count += 1
                    eval_result.append_message("%s is success" %
                                               test_case.get_name())
                else:
                    ...
                    if result is Result.false:
                        ...
                        failure_count += 1
                        eval_result.append_message("%s is error" %
                                                   test_case.get_name())
                    else:
                        _logger.info("Case timeout: %s", test_case.get_name())
                        timeout_count += 1
                        eval_result.append_message("%s is timeout" %
                                                   test_case.get_name())
            except Exception as ex:
        ...
        _logger.info("All done. %d passed, %d failed, %d timeout",
    ...
    def __start_server_if_need(self, clean_data_dir: bool):
        ...
        if self.__need_start_server:
            ...
            miniob_server = MiniObServer(self.__db_server_base_dir,
                                         self.__db_data_dir, self.__db_config,
                                         self.__server_port, unix_socket,
                                         clean_data_dir)
...
def __init_options():
    ...
    options_parser.add_argument('--report-only',
                                action='store_true',
                                dest='report_only',
                                default=False,
    ...
    # 当前miniob的代码目录
    options_parser.add_argument('--project-dir',
                                action='store',
                                dest='project_dir',
                                default='')
    ...
    options_parser.add_argument(
        ...
        dest='test_cases',
        help=
        'test cases. If none, we will iterate the test case directory. Split with \',\' if more than one'
    )
    ...
    options_parser.add_argument(
        ...
        default=False,
        help=
        'If false, server-port will be ignored and will use a random address socket.'
    )
...
def __init_test_suite(options) -> TestSuite:
    ...
    test_suite.set_db_config(
        os.path.abspath(options.project_dir + '/etc/observer.ini'))
...
def __init_test_suite_with_source_code(options, eval_result):
    ...
    if not compile(proj_path, build_path, options.compile_cmake_args,
                   options.compile_make_args, options.compile_rebuild,
                   eval_result):
...
def compile(work_dir: str, build_dir: str, cmake_args: str, make_args: str,
            rebuild_all: bool, eval_result: EvalResult):
    '''
  ...
  '''
    if not os.path.exists(work_dir):
        _logger.error(
            'The work_dir %s doesn\'t exist, please provide a vaild work path.',
            work_dir)
        return False
...
def run(options) -> Tuple[bool, str]:
  ...
  '''
    ...
    try:
        test_suite: TestSuite = __init_test_suite_with_source_code(
            options, eval_result)
